...
export class SlackChannel {
    ...
    setupEventHandlers() {
        this.app.event('message', async ({ event }) => {
            ...
            if (subtype === 'list_record_comment')
                return;
    ...
    async setTyping(jid, isTyping) {
        ...
        if (!messageTs) {
            logger.debug({ jid, isTyping, trackedJids: [...this.lastMessageTs.keys()] }, 'setTyping: no message ts tracked');
            return;
        }
        ...
        try {
            ...
            logger.debug({ err, jid, channelId, messageTs, isTyping }, 'Reaction error');
        }
    }
    /**
